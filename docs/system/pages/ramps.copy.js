...
export const copy = v => ({
  ...
  cap7a: `the control &#183; alpha 0 &#8594; 1 &#183; ${v.band.steepLevels} levels, ${v.band.steepTerrace}px each &#183; smooth`,
  cap7b: `the same endpoints as a <code>background-image</code> &#183; Skia dithers before it quantises &#183; widest run ${v.band.bg}px, column step ${v.band.jumpBg}`,
  cap7: `the shallow ramp as a mask over flat ground &#183; ${v.band.levels} levels, widest terrace ${v.band.terrace}px &#183; column step ${v.band.jumpMask}`,
  cap8: `the mask with <code>.wm-dither</code> on the wrapper &#183; widest run ${v.band.dithered}px, column step ${v.band.jumpDither} &#183; hidden, not removed`,
})
