...
export const copy = v => ({
  ...
  note7: `Eight bits give 256 levels of alpha, and one step is under what a display resolves wherever you put it, so a ramp that spends a run of pixels on each level shows terraces the eye finds before it can name them. <b>More stops cannot help</b>: an 8-stop ramp and a 2-stop ramp quantise to the same levels and band identically, and nothing in CSS asks for more output bits. Sub-level noise is the only control there is, and every band on this page carries it. What governs whether you see it is not the size of the step but the <b>width of the terrace</b>: a one-level boundary is invisible over 4px and an obvious straight edge over 150. The four bands below are the same ink on the same ground through the same quantiser; the first two differ only in slope, the last two are the two ways out. The steep one crosses <b>${v.band.steepLevels}</b> levels and runs <b>${v.band.steepTerrace}px</b> per level, which reads as smooth. The shallow one crosses <b>${v.band.levels}</b> at the same width, which puts <b>${v.band.terrace}px</b> on each &#8212; and those you can count. Dithered, the widest run collapses to <b>${v.band.dithered}px</b>.`,

  note8: `Mostly you do not need it. Skia dithers a <code>background-image</code> gradient and very nearly does not dither a <code>mask-image</code>: the same ramp measures a per-pixel deviation of <b>${v.dither.bg}</b> as a background against <b>${v.dither.mask}</b> as a mask. So a scrim is dithered for you and a mask over a flat ground is not, and that mask is the one place in this package that bands. <code>.wm-dither</code> has to sit <b>after</b> whatever quantises, on the wrapper, not inside the masked element &#8212; inside, the noise modulated the ink before the mask touched it and measurably did nothing. But sitting after the quantiser is also its limit: it composites <b>on top of</b> the steps rather than preventing them, so it breaks the widest run from <b>${v.band.terrace}px</b> to <b>${v.band.dithered}px</b> and hides the edge, while the 1-level step survives in the column mean (<b>${v.band.jumpDither}</b> against the mask's <b>${v.band.jumpMask}</b>) and turning it up adds noise faster than it takes away step. The background-image row is the one that halves the step itself, to <b>${v.band.jumpBg}</b>. <b>None of the three makes an ${v.band.levels}-level ramp over this width disappear</b> &#8212; the fix that works is not needing a ramp this shallow across an area this wide. Judge this chapter on a real display at 100%: a downscaled screenshot averages the terraces away.`,
  ...
  cap7a: `the steep control &#183; alpha 0 &#8594; 1 &#183; ${v.band.steepLevels} levels, widest run ${v.band.steepTerrace}px &#183; smooth`,
  cap7b: `the same two endpoints as a <code>background-image</code> &#183; Skia dithers this one &#183; widest run ${v.band.bg}px, column step ${v.band.jumpBg}`,
  cap7: `the shallow one as a mask over flat ground &#183; ${v.band.levels} levels, widest terrace ${v.band.terrace}px, column step ${v.band.jumpMask}`,
  cap8: `the mask again with <code>.wm-dither</code> on the wrapper &#183; the run breaks to ${v.band.dithered}px, the column step does not (${v.band.jumpDither})`,
})
