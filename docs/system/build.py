...
/* The 6 is cropped on the LEFT by this clip, at the head's own edge -- which sits
   after the rail, so the cut reads as a margin rather than a bleed. Pulling the head
   out by --rail-w and padding it back was tried: it makes the head wider than its
   grid column and the page scrolls sideways. The fix belongs in the grid, not here. */
...
.wm-stack{position:relative;z-index:4;display:flex;flex-direction:column;
  /* Sized to the SHEET, not to a number: it grows until the period of "surface."
     almost reaches the vertical system line on the right edge. That is the measure the
     poster is set to -- the last mark of the type nearly touching the last mark of the
     furniture -- and it is why the vw term does the work while the rem clamps only stop
     it collapsing on a phone or running away on a wall. The rules are .072em, so the
     frame thickens with the words instead of staying a hairline around something loud. */
  margin:clamp(16px,7vh,84px) 24px 0 0;font-size:clamp(3.4rem,14.2vw,16.5rem);line-height:1.0;
