...
(function(){
  ...
  function wireBend(){
    ...
    /* THE SAME FILE THE BAR IS USING, read from --gloss rather than hardcoded. The CSS
       swaps in field-247-sm.avif under 700px -- a smaller texture, because on a phone the
       gloss is driven by SCROLL and every repaint resamples it. The pattern did not know
       that and always asked for the full-size one, so on a narrow viewport the bend was
       sampling a different image from the bar beside it. Same field, different file: they
       cannot be guaranteed to agree, and nothing about the failure announces itself. */
    var gl=getComputedStyle(head).getPropertyValue('--gloss');
    var mu=/url\(\s*['"]?([^'")]+)/.exec(gl);
    im.setAttribute('href', mu ? mu[1] : 'field-247.avif');
  ...
  function place(){
    ...
    /* NOTHING IS MEASURABLE YET -- bail rather than write zeros. Called too early (fonts
       still loading, or a Pages cold load where layout settles later than it does locally)
       the header measures 0 wide, and every value derived from it is 0 too. That shipped:
       the bend's gloss pattern went out at width="0", so the curve had a gloss exactly
       zero pixels across and rendered flat while the bar beside it was lit. Writing
       nothing leaves the last good values in place and lets a later call do the work. */
  ...
  /* And keep watching. A single boot + resize listener is not enough on a cold load: the
     header can gain its size after both have fired -- a webfont swapping in is enough --
     and there is no event for "the thing you measured is finally real". */
  if (window.ResizeObserver) new ResizeObserver(function(){ place(); apply(cu,cv); }).observe(head);
  document.fonts && document.fonts.ready.then(function(){ place(); apply(cu,cv); });
