...
@media (max-width:1080px){
  ...
  /* MOBILE ONLY, and that scoping is the point: every earlier attempt at this went
     into the base rule, and a vw clamp applies at every width -- so tuning a phone
     resized the desktop poster until four lines ran past 100svh and "surface." sat
     below the fold.

     Here the rail is already gone and the edges are 20px, so the headline IS the
     poster: bigger, tighter, and run to the right edge rather than stopping short. */
  .wm-stack{font-size:clamp(3.2rem,15.5vw,7rem);line-height:.96;margin-right:0}
