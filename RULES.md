...
| Every icon name is one the shipped face draws (the face is a 24-ligature subset) | **lint** — `scripts/lint-icons.py` reads the GSUB; package in `lint.yml`, consumers in `consumers.yml`. First catch: the JSDoc example in `Icon.tsx` |
...
| Space: margin on the same scale, a negative step allowed | **lint** — `STEPS`, since 2026-09-25; reported for a day while the 27 sites moved |
