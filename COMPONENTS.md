...
browser	COMPONENTS.md and HOST-CONTRACT.md must match their inputs	2026-09-19T18:48:28.1515301Z +| `ThemeSwitch` | ThemeSwitch -- Auto / Light / Dark, both looks (marks, words) over one engine. The engine is plain JS so a page without React (Kernpare) drives the same control with mountThemeSwitch(); the head-script stamp before first… | font-proofer | GESTURES.md §8 |
...
browser	COMPONENTS.md and HOST-CONTRACT.md must match their inputs	2026-09-19T18:48:28.1537307Z +| `ROLES` `POSTER` `INK` `TRACK_CAPS` `LANDINGS` `typeStyle` `ink` | Type tokens (TYPOGRAPHY.md). type.css is additive — it defines --type-*/--poster-*/ --ink-*/--track-caps and the opt-in `t-*` classes, and styles no element type, so importing it cannot reach existing markup. Apps import… | Kernpare, ReCal, opsz-proofer | TYPOGRAPHY.md |
